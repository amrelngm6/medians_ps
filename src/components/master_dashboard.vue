        ...
        const setCharts = async (data) => {
            ...
            if (content.value.visits_ip_list)
            {
                ...
                for (let i = 0; i < content.value.visits_list.length; i++) {
                    ...
                    events.value.push({ id: i+1, resourceId: i+1, startDate: formatCustomTime(element.created_at, 'YYYY-MM-DD HH:mm:ss'), endDate: formatCustomTime(element.updated_at, 'YYYY-MM-DD HH:mm:ss'), title: (element.item && element.item.item) ? element.item.item.title : '', description: '' },)
                }
