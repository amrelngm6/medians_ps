            ...
            <form action="/api/update" method="POST" data-refresh="1" id="add-device-form" class="action py-0 m-auto rounded-lg max-w-xl pb-10">
                ...
                <div class="py-1 w-full" v-for="column in columns" v-if="columns">
                    ...
                    <div class="w-full" v-if="column" >
                        ...
                        <input :required="column.required" :disabled="column.disabled" v-if="isInput(column.column_type)" autocomplete="off" :name="'params['+column.key+']'" :type="column.column_type" class="h-12 mb-3 rounded w-full border px-3 text-gray-700  focus:border-blue-100 dark:bg-gray-800  dark:border-gray-600" :placeholder="column.title" v-model="item[column.key]">
                    
                        <input :required="column.required" :disabled="column.disabled" v-if="column.column_type == 'password'" autocomplete="off" :name="'params['+column.key+']'" :type="column.column_type" class="h-12 mb-3 rounded w-full border px-3 text-gray-700  focus:border-blue-100 dark:bg-gray-800  dark:border-gray-600" :placeholder="column.title">
                        ...
                        <div v-if="column.column_type == 'checkbox'"  class="py-4 flex gap gap-2 cursor-pointer" @click="setActiveStatus(item, column.key)">
                            ...
                            <input v-if="item[column.key]" v-model="item[column.key]"  type="hidden" class="hidden" :name="'params['+column.key+']'" />
                            <input v-model="item[column.key]"  type="checkbox" class="hidden" :name="'params['+column.key+']'" />
                        </div>
                        
                        <textarea :required="column.required" :disabled="column.disabled" v-if="column.column_type == 'textarea'" :name="'params['+column.key+']'" type="text" rows="4" class="mt-3 rounded-lg w-full border px-3 text-gray-700  focus:border-blue-100 dark:bg-gray-800  dark:border-gray-600" :placeholder="column.title" v-model="item[column.key]"></textarea>

                        <select :required="column.required" :disabled="column.disabled" v-if="column.data && column.column_type == 'select'" v-model="item[column.key]" :name="'params['+column.key+']'" :type="column.column_type" class="h-12 mb-3 rounded w-full border px-3 text-gray-700  focus:border-blue-100 dark:bg-gray-800  dark:border-gray-600"   :placeholder="column.title">
