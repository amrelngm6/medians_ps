            ...
            <form action="/api/update" method="POST" data-refresh="1" id="add-device-form" class="action py-0 m-auto rounded-lg max-w-xl pb-10">
                ...
                <div class="py-1 w-full" v-for="column in columns" v-if="columns">
                    ...
                    <div class="w-full" v-if="column" >
                        ...
                        <vue-medialibrary-field :key="item" v-if="column.column_type == 'file' && column.key != 'profile_image'" :name="'params['+column.key+']'" key="upload-file" v-model="item[picture]" :api_url="conf.url"></vue-medialibrary-field>
                        <vue-medialibrary-field :key="item" v-if="column.column_type == 'file' && column.key == 'profile_image'" :name="'params['+column.key+']'" key="upload-file" v-model="item[profile_image]" :api_url="conf.url"></vue-medialibrary-field>
                    </div>
                </div>
