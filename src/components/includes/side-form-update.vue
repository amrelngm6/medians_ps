        ...
        <div class="p-4 rounded-lg shadow-lg bg-white dark:bg-gray-700" >
            <form action="/api/update" method="POST" data-refresh="1" id="add-device-form" class="action py-0 m-auto rounded-lg max-w-xl pb-10">
                <div class="w-full flex">
                    <h1 class="w-full m-auto max-w-xl text-base mb-10 " v-text="trans('update')"></h1>
                ...
                <div class="py-1 w-full" v-for="column in columns" v-if="columns">
                    ...
                    <div class="w-full" v-if="column" >
                        ...
                        <div v-if="column.column_type == 'checkbox' && !showLoader"  class="py-4 flex gap gap-2 cursor-pointer" @click="setActiveStatus(item, column.key)">
                            ...
                            <span  v-text="item[column.key] ? trans('Active') : trans('Pending')" class=" font-semibold inline-flex items-center px-2 py-1 rounded-full text-xs font-medium "></span>
                        ...
                        <select v-if="column.data && column.column_type == 'select'" v-model="item[column.key]" :name="'params['+column.key+']'" :type="column.column_type" class="h-12 mb-3 rounded w-full border px-3 text-gray-700  focus:border-blue-100 dark:bg-gray-800  dark:border-gray-600"   :placeholder="column.title">
                            <option value="0"  v-if="!column.required" v-text="trans('select') +' '+ column.title"></option>
                        ...
                        <vue-medialibrary-field v-if="column.column_type == 'file' && column.key != 'profile_image'" :name="'params['+column.key+']'" key="upload-file" v-model="item[picture]" :api_url="conf.url"></vue-medialibrary-field>
                        <vue-medialibrary-field v-if="column.column_type == 'file' && column.key == 'profile_image'" :name="'params['+column.key+']'" key="upload-file" v-model="item[profile_image]" :api_url="conf.url"></vue-medialibrary-field>
                    </div>
                </div>

                <button class="uppercase h-12 mt-3 text-white w-full rounded bg-red-700 hover:bg-red-800" v-text="trans('save')"></button>
            </form>
        </div>
    </div>
</template>
