<template>
    <div class="w-full" v-if="column" >
        ...
        <div v-if="column.column_type == 'checkbox'"  class="py-4 flex gap gap-2 cursor-pointer" @click="setActiveStatus(item, column.key)">
            ...
            <input v-if="item[column.key]" v-model="item[column.key]"  type="hidden" class="hidden"  :name="handleName(column)" />
            <input v-model="item[column.key]"  type="checkbox" class="hidden" :name="handleName(column)" />
        </div>
