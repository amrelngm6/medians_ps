                            ...
                            <div  class="w-full grow shrink basis-0 px-6 py-4 flex-col justify-center items-start gap-4 inline-flex" v-if="trip.vehicle">
                                ...
                                <div class="w-full h-8 relative flex">
                                    <div class="w-full relative">
                                        <h3 class="py-2" v-text="__('Waiting pickups')"></h3>
                                        ...
                                        <span class="absolute pt-2" :style="(lang.lang == 'en' ? 'left: ' : 'right: ') +((20 * trip.pickup_locations.length) + 20)+'px'"><i class="fa fa-location-dot text-sm"></i> <span class="font-semibold  px-1" v-if="trip.pickup_locations" v-text="trip.pickup_locations.length"></span></span>
                                    </div>
                                    <div class="w-full relative">
                                        <h3 class="py-2" v-text="__('Active pickups')"></h3>
                                        <img :style="(lang.lang == 'en' ? 'left: ' : 'right: ') +(20 * i)+'px'" class="rounded-full w-8 h-8 left-0 top-0 absolute rounded-[50px] border-2 border-purple-800" v-if="tripLocation.status != 'waiting'" v-for="(tripLocation, i) in trip.pickup_locations" :src="(tripLocation && tripLocation.location && tripLocation.location.picture) ? tripLocation.location.picture : 'https://via.placeholder.com/37x37'" /> 
                                        <span class="absolute pt-2" :style="(lang.lang == 'en' ? 'left: ' : 'right: ') +((20 * trip.pickup_locations.length) + 20)+'px'"><i class="fa fa-location-dot text-sm"></i> <span class="font-semibold  px-1" v-if="trip.pickup_locations" v-text="trip.pickup_locations.length"></span></span>
                                    </div>
