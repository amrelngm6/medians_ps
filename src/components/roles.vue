<template>
    <div class=" w-full pb-20">
        ...
        <side-form-update :conf="conf" model="Role.update" :item="activeItem" :model_id="activeItem.id"
            :index="activeItem.id" v-if="showEditSide && !showAddSide" :columns="content.fillable"
            class="col-md-3" />

        <side-form-create :conf="conf" model="Role.create" v-if="showAddSide && content && content.fillable" :columns="content.fillable"  class="col-md-3" />
    </div>
</template>
<script>
