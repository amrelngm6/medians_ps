	...
	*/
	...
	public function index()
	{
		...
		try {
			...
			$DevicesRepository = new Repo\Devices\DevicesRepository($this->app);

			$today_revenue = (new Repo\Orders\OrdersRepository($this->app))->getByDate(['start'=>date('Y-m-d'), 'end'=>date('Y-m-d')])->sum('total_cost');
            
            $latest_order_products =  (new Repo\Products\StockRepository($this->app))->getLatest(5)->get();

            $today_order_products_count =  (new Repo\Products\StockRepository($this->app))->getLatest(1000)->where('type', 'pull')->where('date', date('Y-m-d'))->count();
